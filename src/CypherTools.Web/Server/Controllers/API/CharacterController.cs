...
namespace CypherTools.Web.Server.Controllers.API
{
    ...
    public class CharacterController : ControllerBase
    {
        ...
        private readonly ILogger<CharacterController> logger = null;

        public CharacterController(CypherContext cypherContext, UserManager<ApplicationUser> userManager, ILogger<CharacterController> logger)
        {
        ...
        public async Task<List<Character>> GetAllUserCharacters()
        {
            var user = await userManager.GetUserAsync(this.User);

            var lsCharacters = cypherContext.Characters.Where(x => x.UserId == Guid.Parse(user.Id) || x.Player == user.DiscordID);

            return lsCharacters.ToList();
        }
            ...
            if (existingModel != null)
            {
                try
                {
                    ...
                    foreach (var recoveryRoll in character.RecoveryRolls)
                    {
                        ...
                        if (recoveryRoll.RecoveryRollId == 0)
                        {
                            existingRecoveryRoll = null;
                        }
        ...
        public async Task<IActionResult> CreateCharacter([FromBody] Character character)
        {
            ...
            character.UserId = Guid.Parse(user.Id);
            character.Player = user.DiscordID;
